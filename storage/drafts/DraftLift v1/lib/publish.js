  ...
  try {
    ...
    if (pushOk) {
      ...
      await appendAudit("publish_fail_git", {
        name: it.name,
        error: "Git push failed or was skipped",
      });
      return { copied: true, pushed: false, error: "Git push failed" };
    }
  } catch (err) {
    ...
    for (const [index, it] of schedule.entries()) {
      ...
      if (t <= Date.now()) {
        ...
        if (publishResult.copied && publishResult.pushed) {
          ...
          changed = true;
          publishedCount++; // Yine de yayınlanmış sayılır, sadece Git'e gitmedi.
          console.warn(
