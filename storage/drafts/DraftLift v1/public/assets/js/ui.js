...
export function renderSchedule(schedule, freshNames, editingName) {
  ...
  items.sort((a, b) => {
    ...
    if (s === "date_desc") {
      // ▼▼▼ YENİ EŞİTLİK BOZUCU MANTIK ▼▼▼

      // 1. Projelerin "yeni" olup olmadığını kontrol et
      const aIsFresh = freshNames.has(a.name);
      const bIsFresh = freshNames.has(b.name);

      // 2. Eğer biri yeni, diğeri eskiyse, yeniyi en üste koy (-1 üste taşır)
      if (aIsFresh && !bIsFresh) return -1;
      if (!aIsFresh && bIsFresh) return 1;

      // 3. Eğer her ikisi de yeni veya her ikisi de eski ise, o zaman tarihe göre sırala
  ...
  items.forEach((it) => {
    ...
    if (freshNames.has(it.name)) {
      tr.classList.add("new-row-highlight");
      // Bu işareti 3 saniye sonra kaldırarak animasyonun tekrar etmesini önle
      setTimeout(() => freshNames.delete(it.name), 3000);
    }
    tr.innerHTML = `
    ...
    if (isRowEditing || (isRowFresh && !isRowPublished)) {
      ...
      // ▼▼▼ YENİ VE BİRLEŞTİRİLMİŞ KONTROL ▼▼▼
      // EĞER PROJE YAYINLANMIŞSA, HEM UPDATE HEM DE DELETE BUTONUNU DEVRE DIŞI BIRAK
      if (isRowPublished) {
        btnUpdate.disabled = true;
        btnDelete.disabled = true;

        // Kullanıcıya neden tıklayamadığını fare ile üzerine gelince gösterelim
        btnDelete.title = "Published projects cannot be deleted.";
      }
      // ▲▲▲ KONTROL BİTTİ ▲▲▲
    }
    // =======================================================================
